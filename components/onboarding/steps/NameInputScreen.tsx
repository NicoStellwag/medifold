...
export const NameInputScreen = () => {
  ...
  // TODO: Consider moving name state to context if needed elsewhere
  const [name, setName] = useState('');

  const handleContinue = () => {
    if (name.trim()) { // Only proceed if name is entered
      // TODO: Save name (e.g., to context state or API call)
      console.log("Name:", name);
      nextStep();
    }
    // Optional: Add validation feedback if name is empty
  };

  const handleSkip = () => {
    // Find the index of the email step and skip to it
        ...
        >
          <CardHeader className="items-center text-center p-6 pt-8">
            ...
            <motion.div variants={item}>
              <CardTitle className="text-2xl font-bold text-cyan-600 mb-2">Let's Get Personal</CardTitle>
            </motion.div>
            <motion.div variants={item}>
              <CardDescription className="text-gray-600">Tell us your name so we can personalize your experience</CardDescription>
            </motion.div>
          </CardHeader>
